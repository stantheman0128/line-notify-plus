...
class ReplyRelayReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (chatTitle != null && service != null) {
            service.handleUserReply(chatTitle, notifTag, notifId, replyText)
        } else if (notifId != -1) {
            // 後備：service 沒在跑 → 至少關掉被回覆的這則，停 spinner
            val manager = context.getSystemService(Context.NOTIFICATION_SERVICE) as NotificationManager
            if (notifTag == null) manager.cancel(notifId) else manager.cancel(notifTag, notifId)
        }
    }
    ...
    companion object {
        ...
        const val EXTRA_NOTIF_TAG = "notif_tag"
